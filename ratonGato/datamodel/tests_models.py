...
class GameModelTests(tests.BaseModelTest):
    ...
    def test4(self):
        ...
        for state in states:
            game = Game(
                cat_user=self.users[0], mouse_user=self.users[1], status=state["status"])
            game.full_clean()
            game.save()
    ...
    def test6(self):
        ...
        for status in [GameStatus.CREATED,
    ...
    def test8(self):
        ...
        for id_cell in [Game.MIN_CELL - 1, Game.MAX_CELL + 1]:
            with self.assertRaises(ValidationError):
                game = Game(
                    cat_user=self.users[0], mouse_user=self.users[1], cat1=id_cell)
                game.full_clean()
            with self.assertRaises(ValidationError):
                game = Game(
                    cat_user=self.users[0], mouse_user=self.users[1], cat2=id_cell)
                game.full_clean()
            with self.assertRaises(ValidationError):
                game = Game(
                    cat_user=self.users[0], mouse_user=self.users[1], cat3=id_cell)
                game.full_clean()
            with self.assertRaises(ValidationError):
                game = Game(
                    cat_user=self.users[0], mouse_user=self.users[1], cat4=id_cell)
                game.full_clean()
            with self.assertRaises(ValidationError):
                game = Game(
                    cat_user=self.users[0], mouse_user=self.users[1], mouse=id_cell)
                game.full_clean()
    ...
    def test9(self):
        ...
        self.assertEqual(User.objects.filter(
            games_as_cat__cat_user__username=self.users[0].username).count(), 0)
        self.assertEqual(User.objects.filter(
            games_as_mouse__mouse_user__username=self.users[1].username).count(), 0)
        ...
        self.assertEqual(User.objects.filter(
            games_as_cat__cat_user__username=self.users[0].username).count(), 1)
        self.assertEqual(User.objects.filter(
            games_as_mouse__mouse_user__username=self.users[1].username).count(), 1)
        ...
        self.assertEqual(self.users[1].games_as_mouse.count(), 1)
        self.assertEqual(User.objects.filter(
            games_as_cat__cat_user__username=self.users[0].username).count(), 2)
        self.assertEqual(User.objects.filter(
            games_as_mouse__mouse_user__username=self.users[1].username).count(), 1)
    ...
    def test11(self):
        ...
        self.assertEqual(
            str(game), "(0, Created)\tCat [X] cat_user_test(0, 2, 4, 6)")
...
class MoveModelTests(tests.BaseModelTest):
    ...
    def test2(self):
        ...
        with self.assertRaisesRegex(ValidationError, tests.MSG_ERROR_MOVE):
            Move.objects.create(
                game=game, player=self.users[0], origin=0, target=9)
