...
class GameMoveTests(tests.BaseModelTest):
    ...
    def test2(self):
        ...
        for move in moves:
            ...
            n_moves += 1
            self.assertEqual(move["positions"],
                             self.get_array_positions(self.game))
    ...
    def test5(self):
        ...
        confs = [
            {"player": self.users[1], "origin": 59,
                "target": 50, "positions": [0, 2, 4, 6, 50]},
            {"player": self.users[0], "origin": 0,
                "target": 9, "positions": [9, 2, 4, 6, 50]},
        ]
            ...
            for _ in range(1, 3):
                if fail:
                    ...
                    Move.objects.create(
                        game=self.game, player=conf["player"], origin=conf["origin"], target=conf["target"])
                    n_moves += 1
                    self.assertEqual(self.get_array_positions(
                        self.game), conf["positions"])
                    self.assertEqual(self.game.cat_turn,
                                     conf["player"] == self.users[1])
                self.assertEqual(self.game.moves.count(), n_moves)
    ...
    def test6(self):
        ...
        Move.objects.create(
            game=self.game, player=self.users[0], origin=0, target=9)
        self.assertEqual(self.game.moves.count(), 1)
        Move.objects.create(
            game=self.game, player=self.users[1], origin=59, target=52)
        ...
        with self.assertRaisesRegex(ValidationError, tests.MSG_ERROR_MOVE):
            Move.objects.create(
                game=self.game, player=self.users[0], origin=9, target=16)
        self.assertEqual(self.game.moves.count(), 2)
        self.game.cat_turn = False
        self.game.save()
        with self.assertRaisesRegex(ValidationError, tests.MSG_ERROR_MOVE):
            Move.objects.create(
                game=self.game, player=self.users[1], origin=52, target=59)
        self.assertEqual(self.game.moves.count(), 2)
