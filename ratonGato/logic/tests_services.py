"""
ChangeLog:
change TestCase by TransactionTestCase
...
class LogInOutServiceTests(ServiceBaseTest):
    ...
    def test2(self):
        ...
        self.client1.post(reverse(LOGIN_SERVICE),
                          self.paramsUser1, follow=True)
        self.assertEqual(
            Decimal(self.client1.session.get(USER_SESSION_ID)), self.user1.id)
    ...
    def test4(self):
        ...
        for session in sessions:
            response = session["client"].get(
                reverse(LANDING_PAGE), follow=True)
    ...
    def test5(self):
        ...
        response = self.client1.post(
            reverse(LOGIN_SERVICE), self.paramsUser1, follow=True)
...
class SignupServiceTests(ServiceBaseTest):
    ...
    def test2(self):
        ...
        self.client1.post(reverse(SIGNUP_SERVICE),
                          self.paramsUser1, follow=True)
    ...
    def test3(self):
        ...
        response = self.client1.post(
            reverse(SIGNUP_SERVICE), self.paramsUser1, follow=True)
    ...
    def test4(self):
        ...
        User.objects.create(
            username=self.paramsUser1["username"], password=pass_dummy)
        n_user = User.objects.count()
    ...
    def test2(self):
        ...
        for session in sessions:
            ...
            id_game = max(game.id for game in list(
                filter(lambda g: g.cat_user != session["user"], games)))
            self.assertEqual(Game.objects.get(
                id=id_game).mouse_user, session["user"])
...
class SelectGameServiceTests(GameRequiredBaseServiceTests):
    ...
    def test3(self):
        ...
        Game.objects.create(cat_user=self.user1,
                            mouse_user=self.user2, status=GameStatus.FINISHED)
        game = Game.objects.create(
            cat_user=self.user1, mouse_user=self.user2, status=GameStatus.ACTIVE)
        Game.objects.create(cat_user=self.user2)
        Game.objects.create(cat_user=self.user2,
                            mouse_user=self.user1, status=GameStatus.FINISHED)
    ...
    def test4(self):
        ...
        self.client1.get(reverse(SELECT_GAME_SERVICE, kwargs={
                         'game_id': game.id}), follow=True)
        self.assertEqual(Decimal(self.client1.session.get(
            constants.GAME_SELECTED_SESSION_ID)), game.id)

        self.loginTestUser(self.client2, self.user2)
        self.assertFalse(self.client2.session.get(
            constants.GAME_SELECTED_SESSION_ID, False))
        self.client2.get(reverse(SELECT_GAME_SERVICE, kwargs={
                         'game_id': game.id}), follow=True)
        self.assertEqual(Decimal(self.client2.session.get(
            constants.GAME_SELECTED_SESSION_ID)), game.id)
    ...
    def test5(self):
        ...
        if id_max is None:
            id_max = 0
        self.loginTestUser(self.client1, self.user1)
        response = self.client1.get(reverse(SELECT_GAME_SERVICE, kwargs={
                                    'game_id': id_max + 1}), follow=True)
    ...
    def test7(self):
        ...
        try:
            user = User.objects.create_user(
                username="dummy_test2", password="dummy_test2")
            game = Game.objects.create(
                cat_user=self.user1, mouse_user=self.user2, status=GameStatus.ACTIVE)
    ...
    def test2(self):
        ...
        for move in moves:
            ...
            response = self.client1.get(
                reverse(SHOW_GAME_SERVICE), follow=True)
...
class MoveServiceTests(PlayGameBaseServiceTests):
    ...
    def test0(self):
        ...
        self.assertFalse(forms.MoveForm(
            {"origin": -1, "target": 0}).is_valid())
        self.assertFalse(forms.MoveForm(
            {"origin": 64, "target": 0}).is_valid())
        self.assertFalse(forms.MoveForm(
            {"origin": 0, "target": -1}).is_valid())
        self.assertFalse(forms.MoveForm(
            {"origin": 0, "target": 64}).is_valid())
    ...
    def test3(self):
        ...
        moves = [
            {**self.sessions[0], **{"origin": 0,
                                    "target": 9, "positions": [9, 2, 4, 6, 59]}},
            {**self.sessions[1], **{"origin": 59,
                                    "target": 50, "positions": [9, 2, 4, 6, 50]}},
            {**self.sessions[0], **{"origin": 9, "target": 16,
                                    "positions": [16, 2, 4, 6, 50]}},
            {**self.sessions[1], **{"origin": 50,
                                    "target": 41, "positions": [16, 2, 4, 6, 41]}},
        ]

        game_t0 = Game.objects.create(
            cat_user=self.user1, mouse_user=self.user2, status=GameStatus.ACTIVE)
        for session in self.sessions:
            self.set_game_in_session(
                session["client"], session["player"], game_t0.id)

        n_moves = 0
        for move in moves:
            response = move["client"].post(
                reverse(MOVE_SERVICE), move, follow=True)
            self.assertEqual(response.status_code, 200)

            game_t1 = Game.objects.get(id=game_t0.id)
            n_moves += 1
            self.assertNotEqual(str(game_t0), str(game_t1))
            self.assertEqual(BaseModelTest.get_array_positions(
                game_t1), move["positions"])
    ...
    def test5(self):
        ...
        for session in self.sessions:
            self.set_game_in_session(
                session["client"], session["player"], game.id)
